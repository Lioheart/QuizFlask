...
def quiz():
    """Wyświetlenie pytań i odpowiedzi w formie quizu oraz ocena poprawności przesłanych odpowiedzi"""
...
def flash_errors(form):
    """Odczytywanie wszystkich błędów formularza i przygotowanie komunikatów"""
    for field, errors in form.errors.items():
        for error in errors:
            if type(error) is list:
                error = error[0]
            flash('Błąd: {}. Pole: {}'.format(error,
...
def get_or_404(pid):
    ...
    try:
        p = Pytanie.select().where(Pytanie.id == pid).get()
        return p
